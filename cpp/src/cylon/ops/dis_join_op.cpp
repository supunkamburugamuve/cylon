...
//  const int32_t MERGE_OP_ID = 5;
  ...
  split_op = new SplitOp(ctx, schema, LEFT_RELATION, callback, {1000, {config.join_config.GetLeftColumnIdx()}});
  ...
  split_op = new SplitOp(ctx, schema, RIGHT_RELATION, callback, {1000, {config.join_config.GetRightColumnIdx()}});
  shuffle_op->AddChild(split_op);
